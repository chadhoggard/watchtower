    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-sm text-gray-500 truncate max-w-[250px]">{monitor.url}</p>
        </div>
        <StatusBadge status={monitor.current_status} />
      </div>

      <div className="flex items-center gap-3 mt-4">
        <span className={`text-xs px-2 py-0.5 rounded font-medium ${envColors[monitor.environment]}`}>
          {monitor.environment}
        </span>
